...
<div class="bg-white shadow overflow-hidden sm:rounded-md">
    <div class="px-6 py-4 border-b border-gray-200">
        <h3 class="text-lg font-medium text-gray-900">Edit: {{ $product->dish_name }}</h3>
    </div>
    
    <form method="POST" action="{{ route('admin.dashboard.menus.update', $product) }}" enctype="multipart/form-data" class="px-6 py-4 space-y-6">
        @csrf
        @method('PUT')
        ...
        @if($product->image_path)
            <div>
                ...
                <img src="{{ asset('storage/' . $product->image_path) }}" alt="{{ $product->dish_name }}" class="w-32 h-32 object-cover rounded-lg">
            </div>
        @endif
        ...
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">Leave empty to keep current image. Accepted formats: JPEG, PNG, JPG, GIF, WebP. Max size: 2MB</p>
            @error('image')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
