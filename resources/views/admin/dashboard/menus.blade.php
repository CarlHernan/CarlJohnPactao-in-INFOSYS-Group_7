        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
                <div class="flex justify-between mb-4">
                    ...
                    <a href="{{ route('admin.dashboard.menus.create') }}" class="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded text-sm">{{ __('Add Dish') }}</a>
                </div>
                        ...
                        @forelse($products as $product)
                            <tr class="border-b border-gray-100 dark:border-gray-700 text-white">
                                ...
                                <td class="py-2 pr-4 text-white">
                                    ...
                                    @else
                                        <span class="px-2 py-1 bg-red-600 text-white rounded text-xs">No</span>
                                    @endif
                                </td>
                                <td class="py-2 pr-4 text-white">
                                ...
                                <td class="py-2 pr-4 space-x-2 text-white">
                                    <a href="{{ route('admin.dashboard.menus.edit', $product) }}" class="text-indigo-400 hover:text-indigo-300 text-xs">Edit</a>
                                    <form method="POST" action="{{ route('admin.dashboard.menus.destroy', $product) }}" class="inline" onsubmit="return confirm('Are you sure you want to delete this menu item?')">
                                        @csrf
                                        @method('DELETE')
